...
class PaypalTagLib {
    ...
    def button = { attrs ->
        ...
        if(!config.server || !config.email) {
            out << '<div style="border:1px solid red; padding:5px;">Paypal is misconfigured. You need to specify the "grails.paypal.server" and "grails.paypal.email" variables</div>'
        }
